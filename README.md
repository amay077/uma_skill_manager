...
ゲーム「ウマ娘 プリティーダービー」のスキルおよびサポートカード（サポカ）を管理するデータ基盤

## プロジェクト情報

- **プロジェクトキー**: `USM`
- **課題管理**: GitHub Issues
- **リポジトリ**: `amay077/uma-skill-manager`
...
|------|------|-----|
| GitHub Issue | `#N` | `#1`, `#2` |
| 課題キー（OpenSpec change-id） | `USM-NNN` | `USM-001`, `USM-002` |
| コミットメッセージ接頭辞 | `#N` | `#1 ログイン機能を実装` |

- コミットメッセージには GitHub Issue 番号（`#N`）を使用（auto-link のため）
- OpenSpec の change-id は `USM-NNN`（0 パディング）形式
- GitHub Issue `#1` は OpenSpec `USM-001` に対応

## 概要

本プロジェクトは、Claude との対話を通じて以下の機能を提供する:
...
4. **Phase 4**: MCP サーバー化（将来構想）
   - Claude Code から直接 DB アクセス
   - 自然言語でのクエリ実行

## Git ブランチ運用

### ブランチ構成
- **main**：本番/リリースブランチ（メインブランチ）
- **feature/USM-XXXX**：機能ブランチ

### 運用方針
- feature ブランチは `main` から分岐し、完了後に `main` へマージします
- **注意**：使用していないブランチから誤って作業ブランチを作成しないでください

## AI 開発環境

このプロジェクトでは AI エージェントを活用した開発を行っています。

### AI エージェント設定

- [CLAUDE.md](./CLAUDE.md): AI エージェントの動作規約
- [AGENTS.md](./AGENTS.md): AI エージェントの動作規約（同一内容）

### 変更提案（OpenSpec）

大きな変更を行う際は、OpenSpec を使用して変更提案を作成してください:

- [OpenSpec ガイド](./openspec/AGENTS.md)
