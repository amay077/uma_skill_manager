...
`umasim_skill.txt` からスキルデータを読み込み、構造化されたデータとして出力するパーサー。

### データ形式

入力ファイルは空行で区切られたスキルブロックで構成される。各ブロックは以下の形式:
...
> **注意**: 目標速度・加速などの数値はゲーム内部の相対値であり、実際の速度単位（m/s など）ではない。パーサーはこれらの値をそのまま整数または浮動小数点数として抽出する。
