...
スキルが持つ複数の効果パターンを表現するためのデータ構造とパース処理を定義する。

### 効果パターンの分類

| パターン | 説明 | 例 |
|---------|------|-----|
| 条件分岐効果 | 追加条件で効果量が変わる（同一発動の異なるバリエーション） | きせき・おもい・かなで |
| 連続発動効果 | A の後 B で発動（1 回の判定で 1 効果） | 全力 V サインッ！ |
| 多段発動効果 | 1 回目の発動が 2 回目以降のトリガーとなる（N 段発動） | ミンナノアタシへ！、エンジン点火！ |

### 多段発動効果の詳細

多段発動効果は `effect_order` で発動順序を管理し、`is_demerit` でデメリット効果かどうかを区別する：

| スキル例 | 1 回目 (effect_order=0) | 2 回目 (effect_order=1) | 備考 |
|---------|------------------------|------------------------|------|
| ミンナノアタシへ！ | 速度 3500（+効果） | 速度 2500（+効果） | 両方メリット |
| エンジン点火！ | 速度 500（+効果） | 現在速度 -500（−効果、is_demerit=true） | 2 回目がデメリット |
| （将来の 3 段発動スキル） | effect_order=0 | effect_order=1 | effect_order=2 も可能 |
